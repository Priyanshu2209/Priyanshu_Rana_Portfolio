...
export default function Portfolio() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Submitting form with data:', { ...formData, message: formData.message.substring(0, 50) + '...' });
        ...
        <div className="fixed inset-0 z-0">
          ...
          <div>
            {stars.small.map((style, i) => (
              <div key={`star-small-${i}`} className="absolute bg-white rounded-full animate-pulse" style={style} />
            ))}
            {stars.medium.map((style, i) => (
              <div key={`star-medium-${i}`} className="absolute bg-blue-100 rounded-full animate-pulse" style={style} />
            ))}
            {stars.large.map((style, i) => (
              <div key={`star-large-${i}`} className="absolute bg-white rounded-full animate-pulse" style={style} />
            ))}
          </div>
        </div>
